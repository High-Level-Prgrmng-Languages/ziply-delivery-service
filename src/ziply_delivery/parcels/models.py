...
class Parcel(models.Model):
    ...
    sender_address = djongo_models.EmbeddedField(model_container=Address)
    
    # Recipient information
    recipient_name = models.CharField(max_length=255)
    recipient_address = djongo_models.EmbeddedField(model_container=Address)
    
    # Current status and location
    current_status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending')
    current_location_lat = models.FloatField(null=True, blank=True)  # GPS latitude
    current_location_lng = models.FloatField(null=True, blank=True)  # GPS longitude
    current_location_address = models.CharField(max_length=255, blank=True)  # Human-readable address
    
    # Timestamps
    created_at = models.DateTimeField(auto_now_add=True)  # When parcel was created
    estimated_delivery = models.DateTimeField(null=True, blank=True)  # Expected delivery time
    
    # Status history - array of embedded documents
    status_history = djongo_models.ArrayField(model_container=StatusHistory, default=list)
